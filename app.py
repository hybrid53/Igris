...
# Reading the data
df = pd.read_csv(r"data/HRDataset_v14.csv")
...
# Creating filter for Performance Score
score_filter = st.sidebar.multiselect("Choose Performance Score",
...
if not dept_filter and not emp_filter and not score_filter:
    ...
    df_final = df3[(df3["EmploymentStatus"].isin(emp_filter)) &
                   (df3["PerformanceScore"].isin(score_filter))]
elif dept_filter:
    df_final = df3[df3["Department"].isin(dept_filter)]
elif emp_filter:
...
#---------------------------------Demographic Filters---------------------------------
if filter_type == "Demographic Filters":
...
with col1:
    st.markdown(f'<div style="padding:15px; text-align:center; color:white;">'
...
with bar1:
    ...
    ax.sex_yticklabels(dept_num.index)
...
with bar2:
    ...
    ax.set_xlabel("Counts")
    ax.set_ylabel("Race")
    ax.set_title("Total Employees by Race")
    st.pyplot(fig5)
...
with bar3:
    ...
    fig6, ax = plt.subplots(figsize=(6.6,4.6))
...
# Trends
# Extract month year from hire date
...
with hire1:
    hire_year = df_final.groupby("DateofHire_year").size().reset_index(name="Count").sort_values(by='DateofHire_year')
    
    hire_year_filtered = hire_year[hire_year['count'] >= 0]

    fig8, ax = plt.subplots(figsize=(8,4))

    ax.fill_between(hire_year_filtered['DateofHire_year'],
                    hire_year_filtered['Count'], color="skyblue")
    ax.plot(hire_year_filtered['DateofHire_year'],
            hire_year_filtered['Count'], color="blue", marker='o')
    ax.set_title("Annual Hire Rate Trend")
    ax.set_xlabel("Year")
    ax.set_ylabel("Counts of Hire")
    st.pyplot(fig8)

# bar chart for recruitment source
with hire2:
    ...
    for index, value in enumerate(source.values):
        ax.text(value, index, str(value), va='center',
                fontsize=10, fontweight='bold')
    ax.set_yticklabels(source.index)
    ax.set_xlabel('Counts')
    ax.set_ylabel('Recruitment Source')
    ax.set_title("Total Employees by Recruitment Source")
    st.pyplot(fig9)

    # --- Attribution Rate ---
    # Extract month year from date of termination
    df_final["DateofTermination"] = pd.to_datetime(df_final["DateofTermination"])
    df_final["DateofTermination_month"] = df_final["DateofTermination"].dt.month
    df_final["DateofTermination_year"] = df_final["DateofTermination"].dt.year
    df_final["DateofTermination_month_year"] = df_final["DateofTermination"].dt.strftime("%b-%y")

    trend1, trend2 = st.columns([1,1])

    with trend1:
        atri = df_final.groupby(['termination_Month_year', 'termination_Month', 'termination_Year']).size().reset_index(name="Count").sort_values(by=["Termination_Year", "Termination_Month"])
        
        # Filter out rows with 'count' is 1
        atri_filtered = atri[atri['Count'] > 0]
        fig6, ax = plt.subplots(figsize=(8,3))
        ax.fill_between(atri_filtered['Termination_Month_year'],
                        atri_filtered['Count'], color="skyblue")
        ax.plot(atri_filtered['Termination_Month_year'],
                atri_filtered['Count'], marker='o')
    ...
    with trend2:
        ...
        df_term = df_final[df_final["Term Reason"] != "N/A - still employed"]
        ...
        ax.set_title("Terminate Employees (Attributed Rate) by TermReason")

        st.pyplot(fig7)

        columns_displayed = ["Employee_Name", "EmpID", "Salary", "Position",
                             "State", "Sex", "MaritalDesc", "CitizenDesc", "RaceDesc", "EmploymentStatus", "Department", "ManagerName", "PerformanceScore", "Age"]
